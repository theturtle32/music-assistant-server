...
- `recommendations/`: the recommendations sub-controller (`controller.py`), which aggregates the rows offered by every provider declaring `ProviderFeature.RECOMMENDATIONS`. The library-backed rows are no longer built here — they come from the builtin `recommendations` plugin provider (`providers/recommendations/`), so the controller has no library-specific branch of its own.
