...
`MetaDataController` is composed from three mixins, each in its own module (#4265, #4838), mirroring how `controllers/players/` is organized — all behaviour is still reachable on the single controller instance, the split is purely for managing a large surface:
...
It does **not** call `create_collage_image`. Playlist artwork is the `playlist_metadata` provider's job (#3786), which offers several layout templates. Note the controller still carries its own collage builder in `images.py`, but nothing in enrichment invokes it. See [`playlist_metadata`](#playlist_metadata) below.
...
- **Skipping provider-owned playlists** (`CONF_SKIP_PROVIDER_PLAYLISTS`, default on): a playlist that is neither `builtin` nor `smart_playlist` and already has a provider-supplied thumb is left alone; only playlists whose art it generated itself are regenerated. `_is_our_image` decides that, and the rule it enforces is that ownership is provable rather than assumed: an image is ours only when its `provider` is this instance, or its path is **absolute** and inside `playlist_metadata_images/`. Remote URLs, data URIs and — importantly — every *relative* path belong to someone else, which is what leaves a cover file sitting beside a filesystem playlist in a subfolder intact (#6230).
...
8. **Station logo fallback**: `get_radio_stream_station_image(streamdetails)` returns the station's own logo when track-level lookup yields nothing. It reads the queue's current item but only accepts the image when `current_item.streamdetails is streamdetails` (#6256) — the first in-band metadata of a freshly started station can arrive while the queue still reports the *previous* item as current, and without the identity check the outgoing track's album art would be stored as the station's logo.
