...
- `music/{type}s/get_by_external_id` is registered on every type controller, so an external id is addressable straight from the API. Note it binds `get_library_item_by_external_id` — the **library-only** lookup.
- `get_item_by_external_id(external_id, external_id_type)` is the wider, currently internal-only path (#5110): it tries the library first, then falls back to asking providers. It is `@final`, and it gives up immediately when `external_id_type` is `None`, since a provider lookup needs to know which identifier it is being handed. Provider support is opt-in per media type through `TRACK_BY_EXTERNAL_ID` / `ALBUM_BY_EXTERNAL_ID` / `ARTIST_BY_EXTERNAL_ID`, dispatching to `get_track_by_external_id` / `get_album_by_external_id` / `get_artist_by_external_id` on `MusicProvider`. Separate features per media type let a provider implement only what it actually supports; no in-tree provider implements them yet.
