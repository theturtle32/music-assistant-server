...
- **ICE servers**: three public STUN servers by default — `stun.l.google.com:19302`, `stun.cloudflare.com:3478`, and `stun.home-assistant.io:3478`. When HA Cloud is available its STUN/TURN set replaces them, adding relay for restrictive networks. `get_ice_servers()` works whether or not remote access is enabled.
...
| [`music_assistant/controllers/webserver/README.md`](../../music_assistant/controllers/webserver/README.md) | In-tree overview. Still describes the removed `auth/oauth_status` polling flow and a drifted STUN list — prefer these docs |
